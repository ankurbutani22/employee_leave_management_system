...
export default function TopBar() {
  ...
  const userAvatar = localStorage.getItem('emp_avatar') 
  ...
  return (
    <header className="h-16 md:h-20 bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-slate-200 px-4 md:px-8 flex items-center justify-between md:justify-end transition-all">
      ...
      {/* Mobile Logo (Visible only on mobile - Top Left) */}
      <div className="md:hidden flex items-center gap-2 font-bold text-slate-800">
        <span className="text-indigo-600 text-xl">⚡</span> Portal
      </div>

      <div className="flex items-center gap-3 md:gap-6">
        
        {/* User Info Pill */}
        <div className="flex items-center gap-3 pl-4 py-1.5 pr-1.5 bg-slate-50 border border-slate-100 rounded-full transition-all hover:border-indigo-100 hover:shadow-sm">
          <div className="text-right hidden sm:block leading-tight">
            <p className="text-sm font-bold text-slate-700 truncate max-w-[150px]">{userEmail}</p>
          ...
          {userAvatar && userAvatar !== "undefined" ? (
            <img 
              src={userAvatar} 
              alt="Profile" 
              className="h-8 w-8 md:h-10 md:w-10 rounded-full object-cover border-2 border-white shadow-sm"
            />
          ) : (
            <div className="h-8 w-8 md:h-10 md:w-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm md:text-lg shadow-sm">
