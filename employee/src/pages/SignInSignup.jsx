...
async function handleAuth(e) {
  ...
  try {
    ...
    if (mode === 'signup') {
      ...
      // જો તમે ફાઇલ સિલેક્ટ કરી હોય (Base64 ને બદલે ફાઇલ ઓબ્જેક્ટ વાપરવો વધુ સારો છે)
      if (e.target.image && e.target.image.files[0]) {
        formData.append('image', e.target.image.files[0]);
      }
    }

    const endpoint = mode === 'signin' ? 'login' : ''; // Signup માટે ખાલી '/' છે
    const res = await fetch(`${API_URL}/employees/${endpoint}`, {
      method: 'POST',
      // નોંધ: FormData વાપરતી વખતે 'Content-Type' હેડર જાતે સેટ ન કરવું
